 ...
 * 덮는 창을 body 끝으로 빼낸다.
 *
 * ★ 이게 없으면 화면을 덮는 창이 화면 높이에 갇히지 않는다.
 * `position: fixed`는 조상에 transform·filter가 하나라도 걸려 있으면 뷰포트가 아니라
...
export function Fieldset({
  ...
  if (!expanded) {
    return (
      <section
        id={id}
        className="flex flex-col gap-2 rounded-box border border-line-card bg-surface-sub px-4 py-4 sm:flex-row sm:items-center sm:gap-5 sm:px-6"
      >
        {/* 항목 이름은 폭을 고정해 세로로 줄을 맞춘다 */}
        <h2 className="fs-title shrink-0 sm:w-[148px]">{title}</h2>
        {/*
...
export function ChipPicker({
  ...
  return (
    <Portal>
      ...
      >
        <div
          className="flex h-full max-h-full w-full flex-col gap-3 rounded-none bg-surface p-4 sm:h-auto sm:max-h-[76vh] sm:max-w-[880px] sm:rounded-card sm:p-6"
...
export function ConfirmDialog({
  ...
  return (
    <Portal>
      ...
      >
        <div
          className="mx-4 flex w-full max-w-[380px] flex-col gap-4 rounded-card bg-surface p-6"
          onClick={(e) => e.stopPropagation()}
        >
