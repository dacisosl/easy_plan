...
export function sectionsPrompt(plan: SemesterPlan, subject: Subject): {
  ...
  return {
    ...
    user: [
      ...
      /*
       * 실물 계획서를 보면 A~E가 서로 다른 다섯 문장이 아니라
       * '한 문장의 수행 강도'만 바꾼 형태다. 그 틀을 그대로 지시한다.
       */
      `semesterLevels = 학기단위 성취수준 ${grades}. 다섯을 각각 새로 쓰지 말고 같은 수행을 강도만 달리해 적는다:`,
      `  A 종합적으로 이해하고 다양한 사례에 적용 · B 이해하고 활용 · C 이해 · D 부분적으로 이해 · E 제한적으로 이해`,
      `  즉 뼈대 문장은 같게 두고 '종합적으로 / 충분히 / 일반적으로 / 부분적으로 / 제한적으로'와 서술어만 바꾼다.`,
