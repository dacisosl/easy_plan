 ...
 * 5. 검증 규칙 (검토 기능)
 *
 * 로직 검증 15개는 전부 결정적이다. 통과 못 하면 내려받기를 막는다.
...
export function validate(
  ...
  for (const p of plan.performances) {
    if (p.ratio > rules.perf_area_max) {
      fail(2, {
  ...
  const passed = Array.from({ length: 15 }, (_, i) => i + 1).filter((n) => !failed.has(n))
